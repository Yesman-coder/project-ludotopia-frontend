				...
				<div className="d-flex flex-row justify-content-center m-4">
					<div className="card text-center column p-3 d-flex">
						...
						<div className="card-body">
							...
							<p className="card-text text-muted font-weight-light text-break">
								{store.betId.description}
							</p>
							<a href="#" className="disabled btn btn-success">
							...
							{store.betId.state == "ganador" && (
								<h3 className="text-success">{`Ganó: ${store.betId.winner_sender}`}</h3>
							)}
							{store.betId.state == "ganador" &&
								store.betId.winner_sender != store.betId.sender && (
